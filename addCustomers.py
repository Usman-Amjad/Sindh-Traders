...
class Customers:
    def __init__(self, root):
        super().__init__()
        self.root = root
        self.root.geometry("960x500+220+148")
        ...
        # ===== Title Label =====
        lbl_title = ctk.CTkLabel(self.root, text="Add Customer", font=("Brush Script MT", 50)).pack(side=TOP, fill=X, padx=10, pady=0)

        # ===== Search Frame =====
        self.searchFrame = ctk.CTkFrame(self.root, width=450, height=80)
        self.searchFrame.place(relx=0.01, rely=0.12)

        lbl_search = ctk.CTkLabel(self.searchFrame, text="Search Customer | By Id", font=("times new roman", 15, "bold")).place(x=2, y=5)
        txt_search = ctk.CTkEntry(self.searchFrame, textvariable=self.sc1, font=("goudy old style", 16),
                                  width=160,
                                  height=25,
                                  border_width=2,
                                  corner_radius=5).place(relx=0.01, rely=0.5)

        btn_show_all = ctk.CTkButton(self.searchFrame, text="Show All", command=self.show, font=("goudy old style", 17),
                                     width=100, height=32, border_width=0,
                                     corner_radius=8, cursor="hand2").place(relx=0.65, rely=0.48)
        btn_search = ctk.CTkButton(self.searchFrame, text="Search", command=self.search1, font=("goudy old style", 17),
                                   width=100, height=32, border_width=0,
        ...
        titleFont = ('Agency FB', 22)

        # ------row1
        lbl_cust_id = ctk.CTkLabel(self.frame4, text="Id", font=(titleFont)).place(x=20, y=5)
        txt_cust_id = ctk.CTkEntry(self.frame4, textvariable=self.cust_id, font=("goudy old style", 16)).place(x=120, y=5, width=200)

        # ------row2
        lbl_cust_name = ctk.CTkLabel(self.frame4, text="Name", font=(titleFont)).place(x=20, y=45)
        txt_cust_name = ctk.CTkEntry(self.frame4, textvariable=self.cust_name, font=("goudy old style", 16)).place(x=120, y=45, width=200)

        # ------row3
        lbl_cust_name = ctk.CTkLabel(self.frame4, text="Balance", font=(titleFont)).place(x=20, y=90)
        txt_cust_name = ctk.CTkEntry(self.frame4, textvariable=self.cust_balance, font=("goudy old style", 16), state=DISABLED).place(x=120, y=90, width=200)

        self.lbl_status = ctk.CTkLabel(self.frame4, text="Status", font=(titleFont))
        self.lbl_status.place(x=20, y=135)
        cmbStatus = ttk.Combobox(self.frame4, textvariable=self.varStatus,
                                 values=("Active", "Inactive"), state='readonly', justify=CENTER,
                                 font=("Agency FB", 17))
        cmbStatus.place(x=150, y=170, width=200)
        cmbStatus.current(0)

        # ------row3 Add Button
        bs = ('Agency FB', 20)

        addBtn = ctk.CTkButton(self.frame4, text="Add", font=(bs), cursor="hand2")
        addBtn.place(x=100, y=180, width=100)
        addBtn.bind("<Return>", self.Add)
        addBtn.bind("<ButtonRelease-1>", self.Add)

        updateBtn = ctk.CTkButton(self.frame4, text="Update", font=(bs), cursor="hand2")
        updateBtn.place(x=185, y=180, width=100)
        updateBtn.bind("<Return>", self.Update)
        updateBtn.bind("<ButtonRelease-1>", self.Update)

        deleteBtn = ctk.CTkButton(self.frame4, text="Delete", font=(bs), cursor="hand2")
        deleteBtn.place(x=270, y=180, width=100)
        ...
        style.configure("Treeview.Heading", font=('Constantia', 16))
