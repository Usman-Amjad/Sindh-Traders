...
class shopExpensesNamesList:
    def __init__(self, root):
        ...
        # ===== Title Label =====
        lbl_title = ctk.CTkLabel(self.root, text="Expenses Names", font=("Brush Script MT", 50)).pack(side=TOP, fill=X,
                                                                                                      padx=10, pady=0,
                                                                                                      ipady=0)
        ...
        txt_cust_name = ctk.CTkEntry(self.frame2, textvariable=self.expDesc, font=("goudy old style", 20)).place(x=150,
                                                                                                                 y=45,
                                                                                                                 width=200)

        addBtn = ctk.CTkButton(self.frame2, text="Add", font=('Agency FB', 22), cursor="hand2")
        addBtn.place(x=350, y=42, width=85)
        addBtn.bind("<Return>", self.Add)
        addBtn.bind("<ButtonRelease-1>", self.Add)

        deleteBtn = ctk.CTkButton(self.frame2, text="Delete", font=('Agency FB', 22), cursor="hand2")
        deleteBtn.place(x=430, y=42, width=85)
    ...
    def Add(self, e):
        ...
        cursor = conn.cursor()
        try:
            cursor.execute("INSERT INTO shopExpensesNames(expDesc) VALUES(?)",
                           (self.expDesc.get(),))
            conn.commit()
    ...
    def delete(self, e):
        ...
        try:
            if self.expDesc.get() == "":
                messagebox.showerror("Error", "Please select name from the list", parent=self.root)
            else:
                cur.execute("SELECT * FROM shopExpensesNames WHERE expDesc=?", (self.expDesc.get(),))
                row = cur.fetchone()
                if row is None:
                    ...
                    if op is True:
                        cur.execute("DELETE FROM shopExpensesNames WHERE expDesc=?", (self.expDesc.get(),))
                        con.commit()
    ...
    def show(self):
        ...
        cursor = conn.cursor()
        try:
            cursor.execute("SELECT expID, expDesc FROM shopExpensesNames")
